...
io.on('connection', (socket) => {
    ...
    socket.on('add-contact', async (data: string | { number: string; platform: Platform }) => {
        ...
        const cleanNumber = validation.cleaned;
        ...
        if (platform === 'signal') {
            ...
            const targetJid = createWhatsAppJid(cleanNumber);
            ...
            try {
                ...
                if (result?.exists) {
                    ...
                    try {
                        ...
                        if (contactInfo && contactInfo[0] && 'notify' in contactInfo[0] && contactInfo[0].notify) {
                            contactName = contactInfo[0].notify as string;
                        }
                    } catch (err) {
